...
// Function to return a list of invalid card numbers
...
//Function to  return the companies of invalid credit cards based on the first card digit
const idInvalidCardCompanies = array => {
    ...
    for (let i = 0; i < invalidCardsArray.length; i++) {
            let firstDigit = invalidCardsArray[i][0]
            if (firstDigit in companies){
                let company = companies[firstDigit]
                companiesArray.push(company)
                companiesArray = Array.from(new Set(companiesArray))
        }
        else {
